...
const AUDIT_DIR = path.join(ROOT, 'docs/audits/sinaiticus-english-shadow');
    ...
    for (const item of unresolved) {
      ...
      if (item.operation.type === 'source-only' && sourceCounts.get(normalized) === 1 && candidateCounts.get(normalized) === 1 && matches.length === 1) {
        ...
        if (candidate.contextualGloss && lexicon?.gloss) {
          ...
          decisions.push({ reference: verse.reference, rowId: item.rowId, sinaiticusGreek: item.greek, decision: 'certified', rule: 'SEA-001-UNIQUE-EXACT-TRANSPOSITION', rationale: 'Unique otherwise-unused TAGNT word in the verse has the identical declared comparison form; only sequence position differs.', proposedGloss: candidate.contextualGloss, evidence: { tagntPosition: candidate.position, tagntGreek: candidate.greek, strong: candidate.strong, morphology: candidate.morphology, tbesgLemma: lexicon.lemma, tbesgGloss: lexicon.gloss } });
          continue;
        }
      }
      ...
      decisions.push({ reference: verse.reference, rowId: item.rowId, sinaiticusGreek: item.greek, decision: 'withheld', rule: textualDifference ? 'SEA-W02-TEXTUAL-DIFFERENCE' : 'SEA-W01-NO-UNIQUE-EXACT-COUNTERPART', classification: countClassification(classification), rationale: textualDifference ? 'The aligned TAGNT form is textually different; similarity alone cannot certify its gloss for Sinaiticus.' : 'No unique, otherwise-unused TAGNT word has the identical declared comparison form in this verse.', candidate: item.tagntCandidate || null });
    }
  }
}
...
const output = {
  ...
  policy: 'Sinaiticus remains governing; only explicitly identified deterministic rules may admit an English annotation.',
  sourceHashes: { tagntSha256: sha256(TAGNT_FILE), tbesgSha256: sha256(TBESG_FILE) },
  rules: {
    'SEA-001-UNIQUE-EXACT-TRANSPOSITION': 'Admit only a one-to-one identical normalized form among unused words in the same verse, with TAGNT contextual gloss and a resolving TBESG entry.',
    'SEA-W01-NO-UNIQUE-EXACT-COUNTERPART': 'Withhold where no unique identical unused TAGNT counterpart exists.',
    'SEA-W02-TEXTUAL-DIFFERENCE': 'Withhold a similar but non-identical form pending independent lexical adjudication.',
  },
