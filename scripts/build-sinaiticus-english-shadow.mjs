...
const OUTPUT_DIR = path.join(ROOT, 'docs/audits/sinaiticus-english-shadow');

function sha256(file) { return crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex'); }
function norm(text = '') {
  const value = text.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/ς/g, 'σ').replace(/[^α-ω]/g, '');
  // Comparison-only expansion of the same Sinaiticus nomina-sacra forms used by
  // the certified CNTR shadow. The diplomatic Greek remains unchanged.
  const nominaSacra = {
...
const summary = {
  ...
  policy: 'CNTR-certified live GA 01 base-reading Greek governs; TAGNT supplies contextual English; TBESG/Abbott-Smith must resolve the lexical identifier; OCR excluded',
...
for (const [gospel, tagntBook] of Object.entries(BOOKS)) {
  const totals = { verses: 0, sinaiticusWords: 0, exact: 0, normalized: 0, withheldAlignment: 0, withheldLexicon: 0, tagntVariantWordsSkipped: 0, omittedOrLacunaCells: 0 };
  const verses = [];
  for (const location of pathsFor(gospel)) {
    totals.verses++;
    const live = JSON.parse(fs.readFileSync(location.file, 'utf8'));
    const words = live.rows.flatMap((row, rowIndex) => row.sinaiticus?.type === 'text' ? [{ rowIndex, rowId: row.id, greek: row.sinaiticus.text }] : []);
    totals.sinaiticusWords += words.length;
    totals.omittedOrLacunaCells += live.rows.filter(row => row.sinaiticus?.type === 'omitted' || row.sinaiticus?.type === 'lacuna').length;
  ...
  summary.gospels[gospel] = { totals: { ...totals, certifiable, coveragePercent: totals.sinaiticusWords ? Number((certifiable / totals.sinaiticusWords * 100).toFixed(3)) : 100 }, verses };
