...
const eligible = new Set(['source-longer-than-row-interval', 'row-interval-longer-than-source']);
const groups = new Map();

for (const run of runsAudit.runs.filter((item) => eligible.has(item.classification))) {
...
for (const group of groups.values()) {
  ...
  const stem = crypto.createHash('sha256').update(`unharmonized|${group.key}`).digest('hex').slice(0, 10);
  const rowIds = Array.from({ length: width }, (_, index) => `pap-unharm-${stem}-${index + 1}`);
  ...
  for (const run of group.runs) {
    for (let index = 0; index < run.words.length; index++) {
      ...
      decisions.push({ gospel: run.gospel, reference: run.reference, siglum: run.siglum, sourceToken: word.sourceWord, sourceIndex: word.sourceWord - 1, diplomatic: word.diplomatic, priorClassification: word.priorClassification, targetRowId: rowIds[index], certified: true, adjudication: 'source-attested-unharmonized-papyrus-row-within-certified-context', conditions: word.conditions ?? [], supplied: word.supplied ?? null, contextualBounds: { before: run.before, after: run.after }, priorComparisonInterval: run.intervalRows.map((row) => row.rowId), comparisonAgreementRequired: false, alignmentStatus: 'unharmonized' });
    }
  }
}
...
if (write && decisions.length) {
  ...
  held.status = 'certified-source-complete-with-explicit-unharmonized-rows';
...
console.log(JSON.stringify({ status: write ? 'applied' : 'read-only', contextualGroups: groups.size, sourceWordsAdjudicated: decisions.length, unharmonizedPapyrusRowsCreated: rowsCreated }, null, 2));
