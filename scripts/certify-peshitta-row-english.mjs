...
const files = new Map();
const failures = [];
const totals = { units: 0, groups: 0, syriacRows: 0, englishWords: 0, glossStarts: 0, glossContinuations: 0, failures: 0 };
...
for (const unit of ledger.decisions) {
  ...
  for (const group of unit.groups) {
    ...
    group.syriacRowIds.forEach((rowId, memberIndex) => {
      ...
      const reference = key?.slice(0, key.lastIndexOf('#'));
      ...
      else if (memberIndex > 0 && (cell.gloss.gloss !== '↳' || cell.gloss.spanRole !== 'continuation')) failures.push({ unitId: unit.unitId, groupId: group.groupId, rowId, issue: 'CONTINUATION_MISMATCH' });
...
if (totals.glossStarts !== totals.groups || totals.glossStarts + totals.glossContinuations !== totals.syriacRows) {
  failures.push({ issue: 'DISPLAY_CELL_ACCOUNTING', groups: totals.groups, glossStarts: totals.glossStarts, glossContinuations: totals.glossContinuations, syriacRows: totals.syriacRows });
}
