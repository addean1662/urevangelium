...
const PRIMARY_LEDGER = path.join(ROOT, 'docs/audits/sinaiticus-english-shadow/adjudication-ledger.json');
const OUTPUT = path.join(ROOT, 'docs/audits/sinaiticus-english-shadow/secondary-source-ledger.json');
...
// Reattach the source-native CNTR conditions already certified on every live GA 01 row.
const sourceNative = new Map();
for (const gospel of Object.keys(GOSPELS)) {
  for (const location of pathsFor(gospel)) {
    ...
    for (const row of live.rows.filter((entry) => entry.sinaiticus?.type === 'text')) {
      const provenance = row.sinaiticus.provenance || {};
      const cntr = { diplomatic: provenance.diplomatic || row.sinaiticus.text, comparison: comparison(row.sinaiticus.text), abbreviation: provenance.abbreviation || null, conditions: provenance.conditions || [], supplied: provenance.supplied || null };
      sourceNative.set(gospel + ' ' + reference + '|' + row.id, { intf: { nomenSacrum: cntr.abbreviation === 'nomina-sacra', unclear: cntr.conditions.some((item) => item.kind === 'damaged'), gap: cntr.conditions.some((item) => item.kind === 'missing'), rawXml: '' }, cntr });
    }
  }
}
...
for (const item of withheld) {
  ...
  const key = comparison(item.sinaiticusGreek);
  const sourceNomen = native?.intf.nomenSacrum && native?.cntr?.abbreviation === 'nomina-sacra' ? SOURCE_NOMINA[plain(item.sinaiticusGreek)] : null;
  if (sourceNomen) {
    const lexical = tbesg.get(sourceNomen.strong);
    if (lexical?.gloss) {
      totals.concordantLemma++;
      totals.sourceNativeCertified++;
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-003-CNTR-NOMEN-SACRUM', proposedGloss: lexical.gloss, evidence: { sourceNative: native, expansion: sourceNomen.expansion, strong: sourceNomen.strong, tbesg: lexical } });
      continue;
    }
  }
  if (native?.cntr?.abbreviation === 'numeric' && item.sinaiticusGreek === '͵β') {
    totals.concordantLemma++;
    totals.sourceNativeCertified++;
    decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-004-CNTR-GREEK-NUMERAL', proposedGloss: 'two thousand', evidence: { sourceNative: native, numericSystem: 'Greek thousands mark plus beta', numericValue: 2000 } });
    continue;
  }
  const keys = [key];
  ...
  if (!(morphLemma && proielLemma && morphLemma === proielLemma) && localWinner.length === 1) {
    ...
    if (lexical?.gloss) {
      ...
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-007-TWO-SOURCE-VERSE-LOCAL-LEMMA', proposedGloss: lexical.gloss, evidence: { ...evidence, verseLocal: local, selectedLemma, strong, tbesg: lexical } });
      continue;
    }
  }
  ...
  if (!(morphLemma && proielLemma && morphLemma === proielLemma) && morphSupportsContext && proielSupportsContext && contextualLexical?.gloss) {
    ...
    decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-005-TAGNT-CONTEXT-MORPHGNT-PROIEL-SUPPORTED-LEMMA', proposedGloss: contextualLexical.gloss, evidence: { ...evidence, tagntCandidate: item.candidate, selectedLemma: contextualLexical.lemma, tbesg: contextualLexical } });
    continue;
  }
  ...
  if (proielLemma && !morphEntries.length) {
    ...
    if (proielLexeme?.gloss) {
      ...
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-010-PROIEL-EXACT-SURFACE-MORPHGNT-LEXICON-LEMMA', proposedGloss: proielLexeme.gloss, evidence: { ...evidence, selectedLemma: proielLemma, morphgntLexicon: proielLexeme } });
      continue;
    }
  }
  if (key === 'υμεισ' && morphLemma === 'συ' && tischLemma === 'συ' && proielLemma === 'υμεισ') {
    ...
    if (pronounLexeme?.gloss) {
      ...
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-011-PROIEL-INFLECTED-PRONOUN-HEADWORD-MORPHGNT-TISCHENDORF-LEMMA', proposedGloss: pronounLexeme.gloss, evidence: { ...evidence, selectedLemma: 'συ', proielHeadwordConvention: 'inflected personal-pronoun headword', morphgntLexicon: pronounLexeme } });
      continue;
    }
  }
  ...
  if (!(morphLemma && proielLemma && morphLemma === proielLemma) && tischLemma && (morphSupportsTisch || proielSupportsTisch)) {
    ...
    if (lexical?.gloss) {
      ...
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-006-TISCHENDORF-THIRD-ANNOTATION-LEMMA', proposedGloss: lexical.gloss, evidence: { ...evidence, selectedLemma: matchingTisch[0].lemma, strong, tbesg: lexical, support: { morphgnt: Boolean(morphSupportsTisch), proiel: Boolean(proielSupportsTisch) } } });
      continue;
    }
  }
  if (morphLemma && proielLemma && morphLemma === proielLemma) {
    ...
    if (lexeme?.gloss && lexical?.gloss) {
      ...
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: sourceNativeRule ? 'SEA-003-SOURCE-NATIVE-EXPANSION-CONCORDANT-LEMMA' : 'SEA-002-MORPHGNT-PROIEL-CONCORDANT-LEMMA', proposedGloss: lexical.gloss, evidence: { ...evidence, morphgntLexicon: { lemma: lexeme.lemma, strong, gloss: lexeme.gloss }, tbesg: lexical } });
    } else if (lexeme?.gloss) {
      totals.concordantLemma++;
      totals.morphLexiconBridgeCertified++;
      decisions.push({ ...item, secondaryDecision: 'certified-lexical', rule: 'SEA-008-MORPHGNT-PROIEL-CONCORDANT-LEMMA-MORPHGNT-LEXICON-GLOSS', proposedGloss: lexeme.gloss, evidence: { ...evidence, morphgntLexicon: { lemma: lexeme.lemma, strong, gloss: lexeme.gloss }, tbesg: lexical } });
    } else {
      totals.missingEnglishBridge++;
      decisions.push({ ...item, secondaryDecision: 'withheld', rule: 'SEA-W06-MISSING-ENGLISH-BRIDGE', evidence });
    }
  } else {
    let rule;
    if (morphLemma && proielLemma) { totals.lemmaDisagreement++; rule = 'SEA-W03-LEMMA-DISAGREEMENT'; }
    else if (morphLemma && !proielEntries.length) { totals.morphgntOnly++; rule = 'SEA-W04-MORPHGNT-ONLY'; }
    else if (proielLemma && !morphEntries.length) { totals.proielOnly++; rule = 'SEA-W05-PROIEL-ONLY'; }
    else if (morphEntries.length || proielEntries.length) { totals.ambiguousAnalysis++; rule = 'SEA-W07-AMBIGUOUS-ANALYSIS'; }
    else { totals.noAnalysis++; rule = 'SEA-W08-NO-EXACT-SURFACE-ANALYSIS'; }
...
const output = {
  ...
  policy: 'Exact Sinaiticus surface form, or a conditioned expansion documented by aligned CNTR GA 01 source markup, must receive one lemma from MorphGNT and one identical lemma from PROIEL; MorphGNT lexicon and TBESG must both resolve the English lexical bridge.',
