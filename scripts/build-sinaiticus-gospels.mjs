...
const SOURCE = {
  witness: 'GA 01',
  name: 'CNTR Class 1 transcription of Codex Sinaiticus',
  revision: '4c0e9f94117ec3dc4ae40094aec044bb7a416a53',
  sha256: 'a0812404a5ef2904a06f91f3a535ca30712f8bc0ee5f5388373c440964be2c5f',
  readingLayer: 'base',
};
const GOSPELS = { 40: 'matthew', 41: 'mark', 42: 'luke', 43: 'john' };
const NON_SINAITICUS = ['papyrus', 'coptic', 'vaticanus', 'bezae', 'vulgate', 'peshitta', 'byzantine'];
...
function otherColumns(rows) {
  return rows.map((row) => Object.fromEntries(NON_SINAITICUS.map((column) => [column, row[column] ?? null])));
}
...
const parseErrors = [];
for (const rawLine of fs.readFileSync(path.join(ROOT, 'data/sources/sinaiticus/01.txt'), 'utf8').split(/\r?\n/).filter(Boolean)) {
  try {
...
const outputDir = path.join(ROOT, 'docs/audits/sinaiticus-gospels');
fs.mkdirSync(outputDir, { recursive: true });
const master = {
  status: 'shadow-review-only', generatedAt: new Date().toISOString(), source: SOURCE,
  policy: 'docs/SINAITICUS_EDITORIAL_POLICY.md', parseErrors, gospels: {},
};
...
for (const gospel of Object.values(GOSPELS)) {
  ...
  for (const location of paths) {
    ...
    if (!parsed) {
      totals.coverageGaps++;
      verses.push({ reference: `${gospel} ${reference}`, status: 'no-GA01-source-line', proposedRows: live.rows });
      continue;
    }

    const sourceWords = parsed.baseWords.filter((word) => word.presence !== 'absent');
    const displayed = live.rows.flatMap((row, rowIndex) => row.sinaiticus?.type === 'text' ? [{ rowIndex, rowId: row.id, text: row.sinaiticus.text }] : []);
    ...
    // inventing a witness-only row when Sinaiticus merely had an empty cell.
    for (let start = 0; start < operations.length;) {
      ...
      for (let rowIndex = lower; rowIndex < upper; rowIndex++) {
        if (live.rows[rowIndex].sinaiticus?.type !== 'text') candidates.push(rowIndex);
      }
    ...
    for (let operationIndex = 0; operationIndex < operations.length; operationIndex++) {
      ...
      if (operation.sourceIndex !== null && operation.displayIndex !== null) {
        ...
        proposedRows[display.rowIndex + insertionOffset].sinaiticus = { type: 'text', text: proposedText, provenance: { ...SOURCE, sourceReference: parsed.reference.code, sourceToken: operation.sourceIndex + 1, raw: word.raw, diplomatic: word.diplomatic, abbreviation: word.abbreviation, conditions: word.conditions, supplied: word.supplied, presence: word.presence, normalization: proposedText === word.diplomatic ? [] : ['expand-special-glyphs', 'modern-final-sigma'], verification: 'source-transcription-verified' } };
        ...
        if (reusableRowIndex !== undefined) {
          ...
          target.sinaiticus = { type: 'text', text: proposedText, provenance: { ...SOURCE, sourceReference: parsed.reference.code, sourceToken: operation.sourceIndex + 1, raw: word.raw, diplomatic: word.diplomatic, abbreviation: word.abbreviation, conditions: word.conditions, supplied: word.supplied, presence: word.presence, normalization: proposedText === word.diplomatic ? [] : ['expand-special-glyphs', 'modern-final-sigma'], verification: 'source-transcription-verified' } };
          totals.sourceOnly++;
        ...
        proposedRows.splice(insertAt, 0, { id: `s01-${parsed.reference.code}-${operation.sourceIndex + 1}`, papyrus: { type: 'empty' }, coptic: { type: 'empty' }, vaticanus: { type: 'empty' }, sinaiticus: { type: 'text', text: proposedText, provenance: { ...SOURCE, sourceReference: parsed.reference.code, sourceToken: operation.sourceIndex + 1, raw: word.raw, diplomatic: word.diplomatic, abbreviation: word.abbreviation, conditions: word.conditions, supplied: word.supplied, presence: word.presence, normalization: proposedText === word.diplomatic ? [] : ['expand-special-glyphs', 'modern-final-sigma'], verification: 'source-transcription-verified' } }, bezae: { type: 'empty' }, vulgate: { type: 'empty' }, peshitta: { type: 'empty' }, byzantine: { type: 'empty' } });
        insertionOffset++;
        totals.sourceOnly++;
        ...
        const display = displayed[operation.displayIndex];
        proposedRows[display.rowIndex + insertionOffset].sinaiticus = { type: 'empty' };
        totals.displayOnly++;
    ...
    const existing = proposedRows.filter((row) => !String(row.id).startsWith('s01-'));
    if (hash(otherColumns(live.rows)) !== hash(otherColumns(existing))) invariantErrors.push(`${gospel} ${reference}: non-Sinaiticus mutation`);

    if (changes.length) exceptions.push({ reference: `${gospel} ${reference}`, changes });
    verses.push({ reference: `${gospel} ${reference}`, sourceReference: parsed.reference.code, sourceRaw: parsed.raw, sourceSegments: parsed.segments, baseWords: sourceWords, currentSinaiticus: displayed, proposedRows });
  ...
  master.gospels[gospel] = { totals, invariantErrors, artifact: `docs/audits/sinaiticus-gospels/${gospel}.json` };
}

fs.writeFileSync(path.join(outputDir, 'summary.json'), JSON.stringify(master, null, 2) + '\n');
const markdown = ['# Complete Sinaiticus Gospel Shadow Build', '', `Generated: ${master.generatedAt}`, '', '**Status: shadow only. No live Gospel data was modified.**', '', `Source: GA 01 at CNTR commit \`${SOURCE.revision}\`.`, '', '| Gospel | Live verses | GA 01 records | Aligned | Coverage gaps | Source-only records | Explicit absence | GA 01 words | Exact | Nomina sacra | Orthographic | Ambiguous | Source-only | Display-only | Invariants |', '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|'];
...
markdown.push('', `MES parse errors: ${parseErrors.length}.`, '', 'Coverage gaps mean no GA 01 source line was present; the shadow retains the existing rows but does not certify their Sinaiticus content.', '');
