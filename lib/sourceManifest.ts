...
export const SOURCE_MANIFEST_VERSION = '2026-08-28.2';
...
export const COLUMN_SOURCES: ColumnSourceRecord[] = [
  ...
  },
  {
    ...
    displayedObject: 'Codex Sinaiticus, GA 01, in the pinned CNTR base-reading transcription.', traditionDate: 'The broader Alexandrian stream predates the codex.', witnessOrEditionDate: 'Codex: approximately 350 CE.',
    status: 'source-verified', statusNote: 'Every displayed GA 01 Gospel token is occurrence-certified against the hash-pinned CNTR Class 1 transcription. The selected reading is the original scribe including recorded first-scribe corrections; later correctors a-c are excluded from display but retained in provenance. Anderson 1918 supplies all displayed English verbatim. Deterministic monotonic allocation permits at most three consecutive Anderson words per extant Greek parent and never generates English.', coverage: 'All 3,779 Gospel records classified: 63,917/63,917 Greek source tokens displayed in source order; 14 damaged tokens, 2 supplied tokens, 1,746 nomina sacra, 547 explicit textual-omission cells, and 12,594 comparison gaps. Of 80,540 acquired Anderson words, 80,394 have extant GA 01 parents and 146 are deliberately suppressed where Anderson includes wording absent from the certified Greek or exceeds the three-word parent ceiling. Seven missing printed verse labels are resolved by exact Anderson substring boundaries; John 21:25 retains its 23 Greek words without invented English because Anderson provides no unit.',
    sources: [
      { name: 'CNTR Class 1 transcription of GA 01', role: 'text', localFiles: 'data/sources/sinaiticus/01.txt', version: 'Commit 4c0e9f94117ec3dc4ae40094aec044bb7a416a53; SHA-256 A0812404A5EF2904A06F91F3A535CA30712F8BC0EE5F5388373C440964BE2C5F', license: 'CC BY-SA 4.0', url: 'https://github.com/Center-for-New-Testament-Restoration/transcriptions/tree/4c0e9f94117ec3dc4ae40094aec044bb7a416a53' },
      { name: 'Henry T. Anderson, The New Testament translated from the Sinaitic Manuscript (1918)', role: 'gloss', localFiles: 'data/sources/sinaiticus-english/anderson-1918', version: '208 hash-recorded responses from the Codex Sinaiticus Project public translation endpoint', license: 'Public domain', url: 'https://codexsinaiticus.org/en/project/translation.aspx' },
      { name: 'Codex Sinaiticus Project', role: 'verification', localFiles: 'External manuscript images and project transcription', version: 'Independent visual control remains available by quire, folio, side, column, and line', license: 'CC BY-NC 4.0', url: 'https://www.codexsinaiticus.org/' },
    ],
    rules: ['Display the pinned GA 01 source, never a critical-edition proxy.', 'Select the base reading: original scribe including recorded first-scribe corrections; exclude later a-c correctors from display.', 'Preserve damage, supplied text, abbreviations, correction layers, textual omissions, and comparison gaps as distinct states.', 'Use Anderson 1918 verbatim; the system assigns published words but never translates.', 'Assign zero to three consecutive English words to each extant Greek parent with deterministic monotonic ordering and fixed tie-breaking.', 'Suppress Anderson wording with no extant GA 01 parent rather than masking a manuscript omission.', 'Keep every decision inside its canonical verse.'],
    prohibited: ['Westcott-Hort or another critical text presented as Sinaiticus', 'Combining later corrector hands without attribution', 'Generated or AI-translated English', 'English-only display rows that conceal a GA 01 omission', 'Cross-verse spans', 'Merged or continuation cells', 'Arrow glyphs'], nextAction: 'Retain the pinned source and rerun the source and Anderson certificates whenever Sinaiticus cells, source files, or alignment rules change.',
  },
  {
  ...
  },
  {
    ...
    sources: [
      ...
      { name: 'SEDRA IV lexical evidence, Beth Mardutho', role: 'alignment', localFiles: 'data/sources/peshitta/sedra-inserted-token-evidence.json', version: 'API responses retained with per-form analyses and source URLs', license: 'External lexical evidence; attribution retained', url: 'https://sedra.bethmardutho.org/' },
    ...
    rules: ['Preserve every Syriac source token exactly once, with source-file hash, source occurrence, and stable row identity.', 'Preserve RTL display while allowing a governed shared-row move to expose meaningful cross-tradition correspondence; the move never changes the stored Syriac token sequence.', 'Keep every governing Murdock verse independent and preserve every admitted word verbatim.', 'Assign zero to three contiguous Murdock words to each Syriac parent using fixed evidence priorities and stable tie-breaking.', 'Use SEDRA IV only for atomic lexical headwords and ETCBC/SyrNT for morphology. Cross-tradition placement evidence requires two independent witness families among Greek, Latin, and Coptic; dependent Greek columns count as one family. Murdock remains the sole displayed English authority.', 'Use an explicit English-only expansion only when all honest adjacent parents are at the three-word ceiling; leave a Syriac parent blank only when the published Murdock unit is mathematically shorter than the source-token inventory.', 'Rerun source, English, row-parent, and governed-alignment certificates after any corpus change.'],
    prohibited: ['Claiming BFBS 1905 without an exemplar chain', 'Treating statistical support or phrase context as dictionary equivalence', 'Using proportional English distribution or AI-generated translation', 'Borrowing displayed wording from another tradition', 'Normalizing samekh forms in the archival display', 'Dropping or duplicating source or English words', 'Merged cells, continuation cells, arrows, or cross-verse English spans'], nextAction: 'Seek independent Syriacist review of the 2,726 governed shared-row decisions and the 28 mathematically explicit edge cases while preserving the pinned source and deterministic certificates.',
  },
  {
