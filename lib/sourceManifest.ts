...
export const COLUMN_SOURCES: ColumnSourceRecord[] = [
  ...
  },
  {
    ...
    status: 'source-verified', statusNote: 'All four Gospels are exact and occurrence-complete against the hash-pinned electronic source: 50,477/50,477 Syriac tokens in source order. Alignment relations for all 728 governed Syriac-only rows are certified. Murdock 1851 English is certified in the accompanying Peshitta English cells as bounded row phrases; phrase spans do not imply one-to-one lexical equivalence.', coverage: 'All 3,779 Gospel verse records; 50,477 source tokens and 84,136 admitted Murdock English words accounted exactly once; 23,650 row-phrase groups, 22,007 cross-tradition lexical anchors, 53 whole-unit fallback spans, and zero accounting failures.',
    sources: [
    ...
    rules: ['Preserve every Syriac source token exactly once, in source order, with source-file hash and occurrence provenance.', 'Preserve RTL display without changing logical token order.', 'Certify one-to-one, one-to-many, many-to-one, and source-specific units as distinct alignment relation types.', 'Use Greek, Latin, and Coptic only as alignment evidence; retain Murdock as the sole displayed English authority.', 'Keep unanchored wording inside bounded phrase spans rather than fabricating lexical equivalence.'],
