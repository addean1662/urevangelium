...
export const COLUMN_SOURCES: ColumnSourceRecord[] = [
  ...
  },
  {
    ...
    rules: ['Preserve every Syriac source token exactly once, in source order, with source-file hash and occurrence provenance.', 'Preserve RTL display without changing logical token order.', 'Keep every governing Murdock verse independent.', 'Use Greek, Latin, and Coptic only as alignment evidence; retain Murdock as the sole displayed English authority.', 'Use explicit English-only expansion rows when no Syriac-cell placement is supported; never proportionally distribute phrases, merge cells, or use continuation arrows.'],
    prohibited: ['Claiming BFBS 1905 without an exemplar chain', 'Treating statistical support as dictionary equivalence', 'Using proportional English translations as word-level lexical evidence', 'Borrowing displayed wording from another tradition', 'Normalizing samekh forms in the archival display', 'Dropping source overflow words to fit Greek-derived rows'], nextAction: 'Obtain independent Syriacist review of corpus-supported placements and the seven explicitly corrected Murdock verse markers.',
  },
  {
