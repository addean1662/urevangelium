...
function getLines(): string[] {
  ...
  const filePath = path.join(process.cwd(), 'data/sources/vulgate/VulgClementine.txt');
...
function loadGospel(gospel: Gospel): Map<VerseKey, string[]> {
  ...
  for (const line of lines) {
    ...
    if (inSection && trimmed.startsWith('### ')) break; // next gospel

    if (!inSection) continue;

    // Format: [chapter:verse] Latin text.
    const match = trimmed.match(/^\[(\d+):(\d+)\]\s+(.+)$/);
    if (!match) continue;

    const [, ch, v, text] = match;
    const words = tokeniseLatin(text);
    if (words.length > 0) {
      map.set(`${ch}:${v}`, words);
    }
  }
...
function tokeniseLatin(text: string): string[] {
  return text
    .split(/\s+/)
...
export function getVulgateVerse(gospel: Gospel, chapter: number, verse: number): string[] {
