...
```css
/* EstrangeloEdessa-BM ships weight 400 only; font-weight: 300 has no effect.
   The font renders heavier than Cardo at the same point size, so font-size is
   reduced ~11% from the scale-up applied to match cap-height. */
.font-syriac { font-family: var(--font-syriac); font-size: 1.2em; line-height: 1; }
```

The `font-size: 1.2em` is relative to the parent `<td>`'s `text-lg` (1.125 rem). Two competing corrections are balanced here: Estrangelo Edessa renders optically small (hence the scale-up above 1em) but also renders heavier-stroked than Cardo (hence keeping it below the 1.35em that would purely match cap-height).
